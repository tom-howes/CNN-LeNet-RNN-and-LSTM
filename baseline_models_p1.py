...
class Encoder(nn.Module):
    def __init__(self, input_size: int, embed_size: int, hidden_size:int, num_layers:int=1, rnn_type:str='lstm'):
        ...
        # TODO: Initialize RNN (nn.RNN) or LSTM (nn.LSTM) based on rnn_type
        # Hint: Use batch_first=True for easier handling
        if self.rnn_type == 'rnn':
            self.model = nn.RNN(embed_size, hidden_size, num_layers, batch_first=True)
    ...
    def forward(self, src, src_lengths) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        input_embedding = self.embedding(src)
        # 2. Pack padded sequence (nn.utils.rnn.pack_padded_sequence)
        packed_sequence = nn.utils.rnn.pack_padded_sequence(input_embedding, src_lengths.cpu(), batch_first=True, enforce_sorted=False)
         # 3. Pass through RNN/LSTM
        packed_output, hidden_state = self.model(packed_sequence)
        # 4. Unpack sequence (nn.utils.rnn.pad_packed_sequence)
        output, _ = nn.utils.rnn.pad_packed_sequence(packed_output, batch_first=True, total_length=src.size(1))
...
class Decoder(nn.Module):
    def __init__(self, output_size: int, embed_size:int, hidden_size:int, num_layers:int=1, rnn_type:str='lstm'):
        ...
        if self.rnn_type == 'lstm':
            ...
            raise ValueError("RNN Type must be lstm or rnn")
        # TODO: Initialize output projection layer: from hidden_size to output_size
    ...
    def forward(self, input: torch.Tensor, hidden: torch.Tensor, encoder_outputs: torch.Tensor=None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        # TODO: Implement single step of decoding
        # Steps:
        # 1. Embed input token
        embedded_input = self.embedding(input)
        # 2. Pass through RNN/LSTM, including the hidden state from the previous time step
        output, hidden_state = self.model(embedded_input, hidden)
        # 3. Apply output projection (need to squeeze the rnn output dim=1)
        output_projection = self.output_layer(output.squeeze(1))
        # 4. Return output and hidden state
        return output_projection, hidden_state
